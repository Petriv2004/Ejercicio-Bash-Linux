...
usuario=''
ip=''
archivocopiar="/home/felipe-trivi-o/A/C/D/H/index.html"
rutair='/var/www/html/'
archivo='index.html'
puerto=''

sshpass -p "$clave" ssh "$usuario@$ip" "sudo rm -f $rutair$archivo"
sshpass -p "$clave" scp "$archivocopiar" "$usuario@$ip:$rutair"

apache_status=$(sshpass -p "$clave" ssh "$usuario@$ip" "systemctl is-active apache2")
if [ "$apache_status" == "inactive" ]; then
    sshpass -p "$clave" ssh "$usuario@$ip" "sudo systemctl restart apache2"
    sshpass -p "$clave" ssh "$usuario@$ip" "sudo systemctl start apache2"
    echo "Apache no está corriendo, intentando iniciarlo..."
